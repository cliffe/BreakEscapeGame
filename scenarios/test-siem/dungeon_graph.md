...
# test-siem — Scenario Graph Reference

Test the SIEM Dashboard minigame. A security team has detected an ongoing attack. Review the alert stream, identify critical indicators of compromise, and escalate all critical-severity alerts to prevent a breach.
...
flowchart TD
  ...
  aim_test_siem_triage{{"Test SIEM Dashboard"}}


  class aim_test_siem_triage aim
```
...
flowchart TD
  ...
  soc("Soc")
  rc_siem_console_1{"SIEM Console"}

  soc --> rc_siem_console_1

  class soc room
  class rc_siem_console_1 item
  class node_start start
```
