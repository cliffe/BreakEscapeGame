...
# npc-patrol-lockpick — Scenario Graph Reference

Test scenario for NPC patrol and lockpick detection
...
flowchart TD
  ...
  node_start(("▶"))
  node_start --> patrol_corridor

  door_secure_vault["Secure Vault<br/>Key lock"]
  secure_vault("Secure Vault")
  patrol_corridor("Patrol Corridor")

  door_secure_vault --> secure_vault

  class door_secure_vault lock
  class secure_vault,patrol_corridor room
...
flowchart TD
  ...
  patrol_corridor("Patrol Corridor")
  secure_vault["Secure Vault<br/>(locked)"]

  patrol_corridor --> secure_vault

  class patrol_corridor room
  class secure_vault lock
  class node_start start
```
...
flowchart TD
  ...
  rc_npc_patrol_face_player_2("Patrol + Face Player")
  rc_npc_security_guard_3("Security Guard")
  rc_obj4_4{"Vault Key"}
  rc_obj5_5{"Classified Files"}
  rc_obj6_6{"Vault Key"}

  patrol_corridor --> secure_vault
  patrol_corridor --> rc_obj1_1
  patrol_corridor --> rc_npc_patrol_face_player_2
  patrol_corridor --> rc_npc_security_guard_3
  rc_npc_security_guard_3 --> rc_obj4_4
  secure_vault --> rc_obj5_5
  secure_vault --> rc_obj6_6

  class patrol_corridor room
  class secure_vault lock
  class rc_obj1_1,rc_obj4_4,rc_obj5_5,rc_obj6_6 item
  class rc_npc_patrol_face_player_2,rc_npc_security_guard_3 npc
  class node_start start
```
