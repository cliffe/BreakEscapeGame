...
{ guard_detection_count == 0:
    Agent 0x99: First thing the security log told me: not a single detection. You were a ghost in there. That's a rare badge.
    #complete_task:zero_detection
}
{ guard_detection_count == 1:
    Agent 0x99: Security flagged one contact with the guard. You recovered, but you left a trace.
}
{ guard_detection_count > 1:
    Agent 0x99: Security logged {guard_detection_count} run-ins with the guard. You got the job done, but stealth wasn't the word for it.
}
...
{ (stealth_rating() > 50) && (stealth_rating() <= 80):
...
{ (player_approach() == "aggressive") && (time_taken() < 1800):
...
{ (flags_submitted_count() >= 2) && (flags_submitted_count() < 4):
...
{ found_exploit_catalog() || (flags_submitted_count() >= 4):
...
{ not (found_exploit_catalog()) && (flags_submitted_count() < 4):
...
{ (objectives_completed() >= 4) && (lore_collected() >= 2):
...
Agent 0x99: Zero Day Syndicate is disrupted. Victoria Sterling
{ victoria_fate() == "arrested":
    is in custody
}
{ victoria_fate() == "recruited":
...
{ (handler_trust >= 50) && (handler_trust < 80):
