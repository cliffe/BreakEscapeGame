...
# test-network-segmentation-plan — Scenario Graph Reference

St. Catherine's Hospital is under a ransomware attack. You are in the IT department. The network between corporate and clinical systems was never properly segmented — two ward exceptions and one legacy admin route still bridge both zones. Review the topology and decide whether to sever the link.
...
flowchart TD
  ...
  node_start(("▶"))
  node_start --> it_security_office

  it_security_office("It Security Office")


  class it_security_office room
...
flowchart TD
  ...
  aim_review_network_map{{"Network Isolation Decision"}}


  class aim_review_network_map aim
```
...
flowchart TD
  ...
  it_security_office("It Security Office")
  rc_network_map_terminal_1{"Network Segmentation Map"}

  it_security_office --> rc_network_map_terminal_1

  class it_security_office room
  class rc_network_map_terminal_1 item
  class node_start start
```
