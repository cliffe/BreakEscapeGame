...
# test-network-architecture — Scenario Graph Reference

Riverside Water Treatment Works has suffered a cyber intrusion. You are in the control room. The network architecture diagram is on the wall — review the Purdue Model layout, identify the attack paths through the IT/OT boundary, and understand how the attacker may have reached the process control network.
...
flowchart TD
  ...
  aim_review_architecture{{"Network Architecture Review"}}


  class aim_review_architecture aim
```
...
flowchart TD
  ...
  control_room("Control Room")
  rc_nad_terminal_1{"Network Architecture Diagram"}

  control_room --> rc_nad_terminal_1

  class control_room room
  class rc_nad_terminal_1 item
  class node_start start
```
